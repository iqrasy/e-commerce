...
const url =
	"https://sephora.p.rapidapi.com/us/products/v2/list?categoryId=cat140014&pageSize=60&currentPage=1";
const option = {
	method: "GET",
	headers: {
...
const bathImport = async () => {
	...
	try {
		...
		const result = await db.collection("bath").insertMany(documents);
	} catch (error) {
	} finally {
		client.close();
	}
